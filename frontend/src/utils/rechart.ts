...
export function getLineColorStops(x: {
  ...
  // here we're taking our line offset and fixing bounds
...
export function useChartDomain(
  ...
  const [maxHistoricalDomain, setMaxHistoricalDomain] = useState<number>(0);

  useEffect(() => {
    if (input.length === 0) return;

    const maxDomain = input.reduce((acc, cur) => {
      const values = Object.values(cur.values);
      const max = Math.max(
        ...values.map((v) => {
          const n = Number(v);
          return isNaN(n) ? 0 : n;
    ...
    const maxDomainWithBuffer = Number((maxDomain * 1.1).toFixed(2));

    if (!maxHistoricalDomain || maxDomainWithBuffer > maxHistoricalDomain) {
      setMaxHistoricalDomain(maxDomainWithBuffer);
    }
  }, [input]);
  ...
  const dataMinWithBuffer = (dataMin: number | string) =>
    typeof dataMin === "number" ? Number(Math.floor(dataMin * 0.9).toFixed(2)) : dataMin;
  if (fixedDomain) {
    return fixedDomain as [AxisDomainItem, AxisDomainItem];
  }
  return maxHistoricalDomain && !isNaN(maxHistoricalDomain)
    ? [dataMinWithBuffer, maxHistoricalDomain]
    : [dataMinWithBuffer, "auto"];
}
