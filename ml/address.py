...
def load_address(
    df_path: str = "data/13. Адресный реестр объектов недвижимости города Москвы.xlsx",
):
...
def process_address(df_address, df_target):
    df_target = (
        df_target.set_index("unom")
        .join(df_address[["ADM_AREA", "DISTRICT", "DREG"]], how="left")
        .reset_index()
    )
    df_target["reg_years_ago"] = (
        (df_target["date"] - df_target["DREG"])
        .apply(lambda x: x.total_seconds() / 60 / 60 / 24 / 30 / 12)
        .fillna(-1)
    )
...
def load_processed_address(parquet_path: str = "features/address.parquet"):
